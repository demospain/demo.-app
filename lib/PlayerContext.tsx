...
interface PlayerContextType {
  ...
  registerShuffleLibraryFn: (fn: () => Promise<void>) => void
}
...
export function PlayerProvider({ children }: { children: ReactNode }) {
  ...
  // Función de fetch de biblioteca — se registra desde DashboardClient
  const shuffleLibraryFnRef = useRef<(() => Promise<void>) | null>(null)
  ...
  const registerShuffleLibraryFn = useCallback((fn: () => Promise<void>) => {
    shuffleLibraryFnRef.current = fn
  }, [])

  // Doble click en shuffle: 1 click = proyecto, 2 clicks rápidos = biblioteca
  const shuffleClickTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  const shuffleClickCount    = useRef(0)

  const handleShuffleClick = useCallback(() => {
    shuffleClickCount.current += 1
    if (shuffleClickTimerRef.current) clearTimeout(shuffleClickTimerRef.current)
    shuffleClickTimerRef.current = setTimeout(async () => {
      const clicks = shuffleClickCount.current
      shuffleClickCount.current = 0
      if (clicks >= 2) {
        // Doble click → shuffle biblioteca
        if (shuffleModeRef.current === 'library') {
          setShuffleMode('none'); shuffleModeRef.current = 'none'
          shufflePendingRef.current = []
        } else if (shuffleLibraryFnRef.current) {
          setShuffleLoading(true)
          await shuffleLibraryFnRef.current()
          setShuffleLoading(false)
        }
      } else {
        // Un click → shuffle proyecto
        shuffleProject()
      }
    }, 280)
  }, [shuffleProject])
    setShuffleMode(prev => {
      ...
      // Pendientes: todas menos la actual, ya en orden aleatorio
  ...
  if (!currentTrack) {
    return (
      <PlayerContext.Provider value={{ currentTrack, isPlaying, shuffleMode, playTrack, closePlayer, shuffleProject, playShuffledLibrary, registerShuffleLibraryFn }}>
        {children}
        ...
        <div className="hidden md:flex w-full max-w-[700px] bg-[#13141a]/96 backdrop-blur-xl border border-white/[0.07] rounded-2xl shadow-2xl items-center gap-3 px-4" style={{ height: '64px' }}>
          ...
          <div className="flex items-center flex-shrink-0 gap-1" style={{ width: '220px' }}>
            ...
            <button onClick={handleShuffleClick} className={`w-7 h-7 relative ${btnCls(shuffleMode !== 'none')}`}>
              ...
              {shuffleMode === 'library' && !shuffleLoading && (
                <span className="absolute bottom-0.5 right-0.5 w-1.5 h-1.5 rounded-full bg-[#6E62F5] border border-[#13141a]"/>
              )}
            </button>
