  ...
  const loadTrack = useCallback(async (track: Track, newQueue?: Track[]) => {
    ...
    try {
      ...
      if ('mediaSession' in navigator) {
        ...
        // Proxy para que iOS/Android cargue la imagen desde nuestro dominio
        const artworkSrc = track.coverUrl
          ? `${origin}/api/artwork?url=${encodeURIComponent(track.coverUrl)}`
          : `${origin}/icon-512.png`
        const artwork = [{ src: artworkSrc, sizes: '512x512', type: imgType }]
