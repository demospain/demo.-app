...
export function PlayerProvider({ children }: { children: ReactNode }) {
  ...
  const SWIPE_THRESHOLD = 80 // px para confirmar cambio
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    // Resistencia: no deja pasar de 120px ni en dirección sin canción
    const limited = Math.max(
      hasPrev ? -120 : -20,
      Math.min(hasNext ? 120 : 20, dx)
    )
    setSwipeDx(limited)
  }
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (dx < -SWIPE_THRESHOLD && hasNext) {
      ...
      setTimeout(() => { playNext(); setSwipeDx(0); setSwipeAnim('none') }, 280)
    } else if (dx > SWIPE_THRESHOLD && hasPrev) {
      setSwipeAnim('snap-right')
      setTimeout(() => { playPrev(); setSwipeDx(0); setSwipeAnim('none') }, 280)
    } else {
      setSwipeAnim('return')
      setTimeout(() => { setSwipeDx(0); setSwipeAnim('none') }, 350)
    }
  }

  // Título de la canción adyacente para mostrar durante el swipe
  const prevTrack = hasPrev ? queue[currentIdx - 1] : null
  const nextTrack = hasNext ? queue[currentIdx + 1] : null
  const swipeLabel = swipeDx < -30 && nextTrack ? nextTrack.title
    : swipeDx > 30 && prevTrack ? prevTrack.title
    : null
  const swipeLabelOpacity = Math.min(1, (Math.abs(swipeDx) - 30) / 50)
      ...
      >
        ...
        <div
          ...
          style={{
            transform: `translateX(${
              swipeAnim === 'snap-left'  ? '-110%' :
              swipeAnim === 'snap-right' ? '110%'  :
              `${swipeDx}px`
            })`,
            ...
            opacity: 1 - Math.abs(swipeDx) * 0.003,
          }}
        ...
        >
          <div className="flex items-center gap-3 px-4 py-3 relative">
            ...
            {/* Info con fade al título adyacente */}
            <div className="flex-1 min-w-0 relative">
              {/* Título actual */}
              <p
                className="text-[14px] font-medium text-[#EAE9E6] truncate leading-tight transition-opacity duration-100"
                style={{ opacity: swipeLabel ? 1 - swipeLabelOpacity : 1 }}
              >
                {currentTrack.title}
              </p>
              {/* Título de la canción hacia la que se desliza */}
              {swipeLabel && (
                <p
                  className="text-[14px] font-medium text-[#EAE9E6] truncate leading-tight absolute inset-0"
            ...
            {/* Indicador direccional durante el swipe */}
            {Math.abs(swipeDx) > 30 && (
              <div
                className="absolute right-4 top-1/2 -translate-y-1/2 text-[#555966] pointer-events-none"
                style={{ opacity: swipeLabelOpacity }}
              >
