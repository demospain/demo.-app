...
export function PlayerProvider({ children }: { children: ReactNode }) {
  ...
  // Cola de pendientes en modo aleatorio — canciones que aún no se han reproducido
  ...
  // Elige la siguiente canción en modo aleatorio sin repetir hasta agotar todas
  const pickNextShuffle = useCallback(() => {
    const current = currentTrackRef.current
    // Si la cola de pendientes está vacía, volver a llenarla (nuevo ciclo)
    if (shufflePendingRef.current.length === 0) {
      const all = queueRef.current.filter(t => t.id !== current?.id)
      // Fisher-Yates
  ...
  const shuffleProject = useCallback(() => {
    setShuffleMode(prev => {
      ...
      // Inicializar pendientes: todas menos la actual, ya mezcladas
  ...
  const playShuffledLibrary = useCallback((tracks: Track[]) => {
    ...
    // Pendientes: todas menos la primera (que se va a reproducir ya)
