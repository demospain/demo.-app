...
// Waveform simulado — seed determinista por ID de track
function generateBars(trackId: string, count = 40): number[] {
  let hash = 0
  ...
  return Array.from({ length: count }, (_, i) => {
    const h = Math.abs(Math.sin(hash * (i + 1) * 9301 + 49297) * 233280) % 1
    // Forma de onda: más alta en el centro
    const center = 1 - Math.abs((i / count) - 0.5) * 0.8
    return 0.15 + h * 0.6 * center + 0.1
  })
}
      ...
      {currentTrack && (
        ...
        >
          <div className="max-w-2xl mx-auto pointer-events-auto">
            ...
            {/* Waveform + seek */}
            <div className="px-3 mb-[-2px] relative h-10 flex items-end gap-[2px] cursor-pointer"
              onClick={(e) => {
                if (!duration) return
              ...
              {bars.map((h, i) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      height:     `${h * 100}%`,
                      background: active ? '#6E62F5' : 'rgba(255,255,255,0.12)',
            ...
            <div className="bg-[#1a1c24]/95 backdrop-blur-xl border border-white/[0.08] rounded-2xl px-3 py-2.5 flex items-center gap-3 shadow-2xl">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs font-mono text-[#555966] truncate">
                  {currentTrack.projectTitle ?? 'demo.'}{duration > 0 ? ` · ${fmt(currentTime)} / ${fmt(duration)}` : ''}
                </p>
              </div>
                ...
                >
                  {repeatMode === 'one' ? (
                    ...
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                      ...
                      <text x="7" y="9" fontSize="5" fill="currentColor" fontFamily="monospace" fontWeight="bold">1</text>
                    </svg>
                  ) : (
                    // Repetir todo / sin repetir
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                      <path d="M2 4h10a2 2 0 012 2v2a2 2 0 01-2 2H4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
                      <path d="M4 7L2 10l2 3" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
