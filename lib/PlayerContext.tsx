      ...
      {currentTrack && (
        ...
        >
          <div className="max-w-2xl mx-auto pointer-events-auto">
            ...
            {/* Player pill — layout exacto untitled */}
            <div className="bg-[#1a1c24]/95 backdrop-blur-xl border border-white/[0.08] rounded-2xl shadow-2xl flex items-stretch overflow-hidden" style={{ height: '56px' }}>

              {/* Portada cuadrada a sangre */}
              <div className="w-14 flex-shrink-0 overflow-hidden bg-[#252830]">
              ...
              {/* Título + subtítulo — ancho fijo */}
              <div className="flex flex-col justify-center px-3 flex-shrink-0" style={{ width: '140px' }}>
                <p className="text-[13px] font-semibold text-[#EAE9E6] truncate leading-tight">{currentTrack.title}</p>
                <p className="text-[11px] text-[#555966] truncate mt-0.5">{currentTrack.projectTitle ?? 'demo.'}</p>
              </div>

              {/* Waveform — flex-1, ocupa todo el espacio central, clickeable */}
              <div
                className="flex-1 relative flex items-center cursor-pointer select-none overflow-hidden"
                style={{ gap: '1.5px', padding: '0 4px' }}
                onClick={(e) => {
                  if (!duration) return
                ...
                {bars.map((h, i) => (
                  <div
                    ...
                    style={{
                      width:        '2px',
                      flexShrink:   0,
                      height:       `${Math.max(h * 70, 4)}%`,
                      background:   'rgba(255,255,255,0.28)',
                      borderRadius: '1px',
                    }}
                  />
                ))}
                {/* Línea vertical amarilla de progreso */}
                <div
                  className="absolute top-2 bottom-2 w-[2px] pointer-events-none rounded-full"
                  style={{
                    left:       `calc(${progressPct * 100}% + 4px)`,
                    background: '#F5C518',
                    transition: 'left 0.1s linear',
                  }}
                />
                {/* Tooltip de tiempo — siempre visible encima de la línea */}
                {duration > 0 && (
                  <div
                  ...
                  >
                    <span className="bg-[#F5C518] text-black text-[10px] font-mono font-bold px-1 py-0.5 rounded whitespace-nowrap" style={{ fontSize: '9px' }}>
              ...
              {/* Botones derechos */}
              <div className="flex items-center gap-0 px-2 flex-shrink-0">
                {/* Repetir */}
                <button
                  onClick={cycleRepeat}
                ...
                >
                  {repeatMode === 'one' ? (
                    <svg width="15" height="15" viewBox="0 0 16 16" fill="none">
                      ...
                      <text x="8" y="9.5" fontSize="4.5" fill="currentColor" fontFamily="monospace" fontWeight="bold" textAnchor="middle">1</text>
                    </svg>
                  ) : (
                    <svg width="15" height="15" viewBox="0 0 16 16" fill="none">
                      <path d="M2 4h10a2 2 0 012 2v2a2 2 0 01-2 2H4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-9 h-9 rounded-full bg-[#6E62F5] hover:bg-[#5A4FD4] flex items-center justify-center transition-colors disabled:opacity-40 flex-shrink-0"
                >
                  {loading ? (
