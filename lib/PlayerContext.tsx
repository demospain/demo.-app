...
export function PlayerProvider({ children }: { children: ReactNode }) {
  ...
  const loadTrack = async (track: Track, newQueue?: Track[]) => {
    ...
    try {
      ...
      // Parar lo que suena antes del fetch
      audio.pause()
      audio.src = ''

      // Usar el endpoint GET que hace proxy del audio (el POST devuelve URL directa de R2 que el navegador bloquea)
      const audioUrl = `/api/play-url?path=${encodeURIComponent(track.file_path)}`
      audio.src = audioUrl
      audio.load()
      await audio.play()
