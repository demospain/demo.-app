...
export default function PublicProjectClient({ project, tracks, isLoggedIn, userId, isOwner }: Props) {
  ...
  return (
    <div className={`min-h-screen bg-[#0f1117] flex flex-col ${playingTrack ? 'pb-20' : ''}`}>
      <style>{`
        @keyframes waveBar {
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-6 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-10">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div>
              <h1 className="text-2xl font-medium text-[#F8F7F4] mb-1.5">{project.title}</h1>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-sm text-[#9BA0AD] font-mono">{project.ownerName}</span>
                    ...
                    >
                      {isLoadingTrack ? (
                        <div className="w-5 flex items-end justify-center gap-[2px] h-4 flex-shrink-0">
                          <div className="w-3.5 h-3.5 border border-white/30 border-t-[#7C6FFF] rounded-full animate-spin"/>
                        </div>
