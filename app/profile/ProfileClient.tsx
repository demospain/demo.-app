...
export default function ProfileClient({ userId, email, profile: initialProfile, trackCount }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[#0f1117]">
      ...
      <nav className="h-16 border-b border-white/[0.07] flex items-center justify-between px-8 sticky top-0 z-50 bg-[#0f1117]/90 backdrop-blur-md">
        <a
          href="/dashboard"
          className="w-9 h-9 rounded-xl bg-[#1E2028] hover:bg-[#252830] border border-white/[0.06] flex items-center justify-center transition-colors"
        >
        ...
        <div className="relative" ref={dotsMenuRef}>
          <button
            ...
            className={`w-9 h-9 rounded-xl border flex items-center justify-center transition-colors ${
              showDotsMenu
                ? 'bg-[#1f2335] border-white/[0.1] text-[#EAE9E6]'
                : 'bg-[#1E2028] border-white/[0.06] text-[#555966] hover:text-[#9BA0AD] hover:bg-[#252830]'
            }`}
          >
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          ...
          {showDotsMenu && (
            <>
              ...
              <div className="absolute right-0 top-11 bg-[#1E2028] border border-white/[0.08] rounded-xl shadow-xl z-50 py-1.5 min-w-[180px]">
                <button
                  onClick={async () => {
                    const supabase = createClient()
