...
export default async function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0f1117] flex flex-col">
      ...
      <header className="flex items-center justify-between px-8 h-[60px] border-b border-white/[0.05] sticky top-0 z-50 bg-[#0f1117]/95 backdrop-blur-md">
        <a href="/dashboard" className="font-mono text-2xl font-medium tracking-tight hover:opacity-80 transition-opacity">
          demo<span className="text-[#6E62F5]">.</span>
        </a>
        <div className="flex items-center gap-2">
          <NotificationBell unreadCount={unreadCount} userId={user.id}/>
          <a href="/profile">
            <div className="w-10 h-10 rounded-xl bg-[#6E62F5] flex items-center justify-center text-sm font-bold text-white overflow-hidden border border-white/[0.08] hover:border-white/20 transition-colors">
              {profile?.avatar_url ? (
                <img
