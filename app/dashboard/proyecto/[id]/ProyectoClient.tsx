                ...
                <div className="flex items-center gap-2 mt-3">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      // Dos queries: proyectos propios + proyectos guardados del usuario
                      const [{ data: myTracks }, { data: savedTracks }] = await Promise.all([
                        supabase
                          .from('tracks')
                          .select('id, title, file_path, project_id, projects!inner(id, title, cover_url)')
                      ...
                      const combined = [...(myTracks ?? []), ...(savedTracks ?? [])]
                      if (combined.length === 0) return
                      const allTracks = combined.map((t: any) => ({
                        id:           t.id,
                        title:        t.title,
