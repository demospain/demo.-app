...
export default function ProyectoClient({ project: initialProject, initialTracks, isMine: isOwner, isAdmin, userId, nombre, inicial }: Props) {
  // Los administradores tienen los mismos permisos de edición que el propietario
  // en todo el proyecto (portada, renombrar, subir, borrar, menú de canciones)
  const isMine = isOwner || isAdmin
              ...
              <div className="bg-[#181c27] border border-white/[0.07] rounded-xl overflow-visible">
                ...
                {filteredTracks.map((track, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        onClick={() => playTrack({ id: track.id, title: track.title, file_path: track.file_path, projectTitle: project.title, projectId: project.id, coverUrl: project.cover_url ?? undefined, waveform: track.waveform, artistName: nombre }, filteredTracks.map(t => ({ id: t.id, title: t.title, file_path: t.file_path, projectTitle: project.title, projectId: project.id, coverUrl: project.cover_url ?? undefined, waveform: t.waveform, artistName: nombre })))}
