...
export default function ProyectoClient({ project: initialProject, initialTracks, isMine, userId, nombre, inicial }: Props) {
  ...
  const { currentTrack, playTrack, closePlayer, isPlaying: playerIsPlaying, shuffleMode, shuffleProject, playShuffledLibrary } = usePlayer()
            ...
            <div>
              ...
              {tracks.length > 1 && (
                <div className="flex items-center gap-2 mt-3">
                  ...
                  <button
                    onClick={() => {
                      const q = tracks.map(t => ({ id: t.id, title: t.title, file_path: t.file_path, projectTitle: project.title, coverUrl: project.cover_url ?? undefined }))
                      if (shuffleMode === 'project') {
                        shuffleProject()
                      } else {
                        // Mezclar y reproducir desde el primero
                        const shuffled = [...q].sort(() => Math.random() - 0.5)
                        playTrack(shuffled[0], shuffled)
                        setTimeout(() => shuffleProject(), 50)
                      }
                    }}
                  ...
                  <button
                    onClick={async () => {
                      if (shuffleMode === 'library') {
                        playShuffledLibrary([]) // desactiva
                        return
                      }
                      // Fetch de todos los tracks del usuario
                      const { data } = await supabase
                        .from('tracks')
                        .select('id, title, file_path, projects!inner(id, title, cover_url, owner_id, visibility)')
                        .or(`projects.owner_id.eq.${userId},projects.visibility.eq.public`)
                        .limit(500)
                      ...
                      const allTracks = data.map((t: any) => ({
                        ...
                        coverUrl:     t.projects?.cover_url ? `https://pub-5ad091444ab84f6e979864f025aa8867.r2.dev/${t.projects.cover_url}` : undefined,
                      }))
