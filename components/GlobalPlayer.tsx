...
export default function GlobalPlayer() {
  const { currentTrack, isPlaying, closePlayer, wsRef } = usePlayer()
    ...
    const init = async () => {
      try {
        ...
        ws = WaveSurfer.create({
          ...
          cursorColor:   'rgba(124,111,255,0.6)',
          barWidth:      2,
          barRadius:     2,
          height:        36,
          normalize:     true,
      ...
      <div className="max-w-5xl mx-auto px-5 py-3 flex flex-col gap-2">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={togglePlay}
            disabled={loading}
            className="w-9 h-9 rounded-full bg-[#7C6FFF] hover:bg-[#4A3FCC] flex items-center justify-center flex-shrink-0 transition-colors disabled:opacity-40"
          >
            {loading ? (
