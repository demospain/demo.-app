...
function ActorAvatar({ actor }: { actor: Notification['actor'] }) {
  const inicial = (actor?.username ?? actor?.full_name ?? 'A').charAt(0).toUpperCase()
      ...
      <button
        ...
        className={`relative w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${
          open
            ...
            : 'hover:bg-white text-[#0f1117]'
        }`}
        style={{ backgroundColor: open ? '#6E62F5' : '#F8F7F4' }}
      >
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          <path d="M8 1.5a5 5 0 015 5v2.5l1 2H2l1-2V6.5a5 5 0 015-5z" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
