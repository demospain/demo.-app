...
export default function DashboardClient({ userId, userName, initialProjects, savedProjects, ownerNames }: DashboardClientProps) {
  ...
  const { shuffleMode, playShuffledLibrary, registerShuffleLibraryFn } = usePlayer()
  const shuffleModeRef = useRef(shuffleMode)
  useEffect(() => { shuffleModeRef.current = shuffleMode }, [shuffleMode])

  useEffect(() => {
    registerShuffleLibraryFn(handleShuffleLibrary)
  }, [projects, savedProjects]) // re-registrar cuando cambian los proyectos
  ...
  const handleShuffleLibrary = async () => {
    if (shuffleModeRef.current === 'library') { playShuffledLibrary([]); return }
    setShuffleLoading(true)

    // Query 1: tracks de proyectos propios
    ...
    // Query 2: IDs de proyectos guardados, luego sus tracks
    const { data: savedRows } = await supabase
      .from('saved_projects')
      .select('project_id')
      .eq('user_id', userId)

    const savedIds = (savedRows ?? []).map((r: any) => r.project_id)
    let savedTracks: any[] = []
    if (savedIds.length > 0) {
      const { data } = await supabase
        .from('tracks')
        .select('id, title, file_path, project_id')
        .in('project_id', savedIds)
        .limit(200)
      savedTracks = data ?? []
    }
    ...
    // Mapear cover_url y título desde el estado local
    const projectMap = Object.fromEntries(
      [...projects, ...savedProjects].map(p => [p.id, p])
    )
