...
export default function DashboardClient({ userId, userName, initialProjects, savedProjects, ownerNames }: DashboardClientProps) {
  ...
  const handleShuffleLibrary = async () => {
    ...
    const allProjectIds = [...projects, ...savedProjects].map(p => p.id)
    if (allProjectIds.length === 0) { setShuffleLoading(false); return }
    ...
    // Mapear cover_url y título del proyecto desde el estado local
